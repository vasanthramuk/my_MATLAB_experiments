...
w_n = [];
for n=0:M-1
    w_n(n+1)=0.42-0.5*cos(2*pi*n/(M-1))+0.08*cos(4*pi*n/(M-1));
end
...
title('FIR LPF - Blackman window');

figure(2);
[b,a] = fir1(M-1,2*fc/fsample,'low',blackman(M));
freqz(b,a,fsample,fsample);
title('For checking: FIR LPF - Blackman window');